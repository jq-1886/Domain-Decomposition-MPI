...
// This version of the code is designed to run on a remote HPC with no user visibility of the std::cout or std::cerr channels

int i_max = 1000; // setting the global number of rows
int j_max = 1000; // setting the global number of columns
...
int it_out = 500; // setting at which iteration increment the output is written to file

std::string directory_string; // setting a global string used in directory creation, broadcast with master process id 0

// for developer testing purposes only, user calls are not recommended
...
const std::string get_current_time()
{
    ...
    char time_buffer[80]; // creating a buffer of chars to allow for population of current time in char array format
...
void master_sets_domain()
{
    ...
    // to be deleted
    // int terminal_id = p - 1; // determines the id of the in final process using 0 based indexing, master process has id 0
